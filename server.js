...
const uploadDirect = multer({
  storage: multer.diskStorage({
    destination: (req,file,cb) => {
      const safeSubject = (req.body.subject||'General').replace(/[^a-zA-Z0-9 ]/g,'').trim();
      const safeUnit    = (req.body.unit_name||'Unit1').replace(/[^a-zA-Z0-9 ]/g,'').trim();
      const dir = path.join('uploads','lectures',safeSubject,safeUnit);
      if (!fs.existsSync(dir)) fs.mkdirSync(dir,{recursive:true});
      cb(null,dir);
    },
  ...
  fileFilter:(_,file,cb) => file.mimetype==='application/pdf'?cb(null,true):cb(new Error('Sirf PDF')),
});
...
app.post('/api/auth/register', async (req,res) => {
  ...
  try {
    ...
    if (e.code==='23505') return res.status(409).json({error:'Email already registered hai'});
...
app.get('/api/lecture/count', authMiddleware, requireRole('teacher'), async (req,res) => {
  ...
  try {
    const {rows} = await pool.query(
      `SELECT COUNT(*) as count FROM lectures WHERE teacher_id=$1 AND subject=$2 AND unit_name=$3 AND class_name=$4 AND status='completed'`,
...
app.post('/api/lecture/start', authMiddleware, requireRole('teacher'), async (req,res) => {
  ...
  try {
    ...
    setTimeout(()=>delete captureCodes[code], 6*60*60*1000);
...
app.delete('/api/lecture/:id', authMiddleware, requireRole('teacher'), async (req,res) => {
  try {
    const {rows} = await pool.query('SELECT * FROM lectures WHERE id=$1 AND teacher_id=$2',[req.params.id,req.user.id]);
    if (!rows[0]) return res.status(404).json({error:'Lecture nahi mili'});
    if (rows[0].pdf_path) {
      const fp = path.join(__dirname,rows[0].pdf_path);
...
app.post('/api/lecture/upload', authMiddleware, requireRole('teacher'), uploadDirect.single('pdf'), async (req,res) => {
  ...
  if (!title||!subject||!class_name||!req.file) return res.status(400).json({error:'Sab fields chahiye'});
  try {
    ...
    const lectureNo = parseInt(cr[0].count)+1;
...
app.get('/api/lectures', authMiddleware, async (req,res) => {
  try {
    ...
    if (req.user.role==='teacher') {
      ...
      ({rows}=await pool.query(
        `SELECT * FROM lectures WHERE status='completed' AND (class_name=$1 OR subject=ANY($2))
...
app.post('/api/attendance/start', authMiddleware, requireRole('teacher'), async (req,res) => {
  const {subject,class_name,lecture_id} = req.body;
  if (!subject||!class_name) return res.status(400).json({error:'Subject aur class chahiye'});
  try {
    // Close any existing open session for this teacher
    await pool.query(
    ...
    const qrCode = uuidv4();
    const {rows} = await pool.query(
      `INSERT INTO attendance_sessions (teacher_id,teacher_name,subject,class_name,lecture_id,qr_code,status)
       VALUES ($1,$2,$3,$4,$5,$6,'open') RETURNING *`,
      [req.user.id,req.user.name,subject,class_name,lecture_id||null,qrCode]
    );
    const session = rows[0];
    io.emit('attendance:started',{sessionId:session.id,subject,class_name});
    res.json({success:true,session,qrCode});
  } catch(e){console.error(e);res.status(500).json({error:'Server error'});}
});

// Teacher: Close attendance session
app.post('/api/attendance/close', authMiddleware, requireRole('teacher'), async (req,res) => {
  const {sessionId} = req.body;
  try {
    await pool.query(
      `UPDATE attendance_sessions SET status='closed', closed_at=NOW() WHERE id=$1 AND teacher_id=$2`,
      [sessionId,req.user.id]
    );
    io.emit('attendance:closed',{sessionId});
    res.json({success:true});
  } catch(e){res.status(500).json({error:'Server error'});}
});

// Student: Mark attendance via QR code
app.post('/api/attendance/mark/:qrCode', authMiddleware, requireRole('student'), async (req,res) => {
  const {qrCode} = req.params;
  const {studentIp} = req.body;
  try {
    // Find open session
    const {rows:sessions} = await pool.query(
      `SELECT * FROM attendance_sessions WHERE qr_code=$1 AND status='open'`,
      [qrCode]
    );
    if (!sessions[0]) return res.status(404).json({error:'Session expire ho gayi ya band ho gayi'});
    ...
    if (existing[0]) return res.status(409).json({error:'Aapki attendance pehle se mark hai!'});

    // Mark attendance
    await pool.query(
      `INSERT INTO attendance_records (session_id,student_id,student_name) VALUES ($1,$2,$3)`,
      [session.id,req.user.id,req.user.name]
    );
    ...
    io.emit('attendance:marked',{sessionId:session.id,studentName:req.user.name,count:parseInt(countRows[0].count)});
    res.json({success:true,message:'Attendance mark ho gayi!'});
  } catch(e){console.error(e);res.status(500).json({error:'Server error'});}
});

// Teacher: Get attendance list
app.get('/api/attendance/:sessionId', authMiddleware, requireRole('teacher'), async (req,res) => {
  try {
    const {rows:session} = await pool.query(
      `SELECT * FROM attendance_sessions WHERE id=$1 AND teacher_id=$2`,
      [req.params.sessionId,req.user.id]
    );
    if (!session[0]) return res.status(404).json({error:'Session nahi mili'});
    const {rows:records} = await pool.query(
      `SELECT ar.*, u.class_name FROM attendance_records ar
       JOIN users u ON u.id=ar.student_id
       WHERE ar.session_id=$1 ORDER BY ar.marked_at`,
      [req.params.sessionId]
    );
    res.json({session:session[0],records,count:records.length});
  } catch(e){res.status(500).json({error:'Server error'});}
});

// Teacher: Get all attendance sessions
app.get('/api/attendance', authMiddleware, requireRole('teacher'), async (req,res) => {
  try {
    const {rows} = await pool.query(
      `SELECT s.*, COUNT(r.id) as student_count
...
// Active attendance session for student
app.get('/api/attendance/active/check', authMiddleware, async (req,res) => {
  try {
    const {rows} = await pool.query(
      `SELECT * FROM attendance_sessions WHERE status='open' ORDER BY created_at DESC LIMIT 5`
    );
    res.json(rows);
  } catch(e){res.status(500).json({error:'Server error'});}
});
...
app.get('/',           (_,r) => r.sendFile(pub('landing.html')));
app.get('/app',         (_,r) => r.sendFile(pub('index.html')));
...
app.get('/attend/:qr', (_,r) => r.sendFile(pub('attend.html')));
